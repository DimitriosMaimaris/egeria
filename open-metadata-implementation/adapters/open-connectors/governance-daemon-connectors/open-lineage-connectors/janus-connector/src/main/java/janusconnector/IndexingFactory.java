...
import static janusconnector.utils.GraphConstants.corePropertyTypes;
...
public class IndexingFactory {
    ...
    protected static void createCompositeIndexForVertexProperty(String propertyName, String propertyKeyName, boolean unique, JanusGraph graph)
    {
        ...
        try {
            ...
            log.error("{} class not found so no Index is created for property {}", methodName, propertyName);
        }

        String indexName = "vertexIndexComposite" + propertyKeyName;
        log.info("Index creation with name {}", indexName);
        ...
        try {
            ...
            if (existingIndex != null) {
                log.info("{} this index {} already exists", methodName, indexName);
                management.rollback();
                return;

            } else {
                // index does not already exist - create
                log.info("{} create index {} for vertex property {}", methodName, indexName, propertyKeyName);
            }
            ...
            if (existingPropertyKey != null) {
                ...
                log.debug("{} create property key for property {}", methodName, propertyKeyName);
